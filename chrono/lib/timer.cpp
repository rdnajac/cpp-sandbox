...
void Timer::printLogs() const {
    ...
    std::cout << std::left << std::setw(30) << "Description" << std::setw(20) << "Duration (ms)" << std::endl;

    for (size_t i = 0; i < m_durations.size(); ++i) {
        std::cout << std::left << std::setw(30) << m_labels[i] << std::setw(20) << m_durations[i].count() << std::endl;
    }
}

long long Timer::getDuration(size_t index) const {
    if (index >= m_durations.size()) return 0;
...
void Timer::printResults() const {
    ...
    for (size_t i = 0; i < m_durations.size(); ++i) {
        std::cout << std::left 
                  << std::setw(16) << m_results[i] 
                  << std::setw(32) << m_labels[i] 
